...
function App() {
  ...
  <div style={{
      ...
      <div style={{ position: 'absolute', top: 18, right: 24, zIndex: 100 }}>
        <select value={language} onChange={e => setLanguage(e.target.value)} style={{ padding: '6px 12px', borderRadius: 6, border: '1px solid #bbb', fontSize: 15 }}>
      ...
      <div style={heroSectionStyle}>
        ...
        <div style={heroContentStyle}>
          <h1 style={{ fontSize: 40, margin: 0, color: '#fff', fontWeight: 700, letterSpacing: 2, textShadow: '0 2px 12px #0006' }}>
            <span style={waveAnimStyle}>🌱</span> AgriScan
          </h1>
          <p style={{ fontSize: 22, color: '#e0ffe0', margin: '18px 0 0 0', fontWeight: 500, textShadow: '0 2px 8px #0004' }}>
            Empowering Farmers with <span style={gradientTextStyle}>Smart Insights</span>
          </p>
        </div>
      ...
      {chatOpen && (
        <div style={{
          ...
          <div style={{ padding: '14px 18px', borderBottom: '1px solid #e0e0e0', background: '#388e3c', color: '#fff', borderTopLeftRadius: 16, borderTopRightRadius: 16 }}>
            <b>AgriScan Chatbot</b>
            <button onClick={() => setChatOpen(false)} style={{ float: 'right', background: 'none', border: 'none', color: '#fff', fontSize: 20, cursor: 'pointer' }}>&times;</button>
          </div>
          <div style={{ flex: 1, overflowY: 'auto', padding: 14, background: '#f9fbe7' }}>
            {chatHistory.length === 0 && <div style={{ color: '#888', textAlign: 'center', marginTop: 30 }}>Ask me anything about farming, weather, or market prices!</div>}
            {chatHistory.map((msg, i) => (
              <div key={i} style={{ margin: '8px 0', textAlign: msg.from === 'user' ? 'right' : 'left' }}>
                <span style={{
                  ...
                  maxWidth: '80%',
                  fontSize: 15,
                  boxShadow: msg.from === 'user' ? '0 2px 8px #388e3c22' : '0 2px 8px #fbc02d22',
                }}>{msg.text}</span>
              </div>
            ))}
          </div>
          <form onSubmit={sendChatMessage} style={{ display: 'flex', borderTop: '1px solid #e0e0e0', background: '#fff', borderBottomLeftRadius: 16, borderBottomRightRadius: 16 }}>
            <input
              type="text"
              value={chatInput}
              onChange={e => setChatInput(e.target.value)}
              placeholder="Type your question..."
              style={{ flex: 1, border: 'none', outline: 'none', padding: 12, fontSize: 15, borderBottomLeftRadius: 16 }}
            />
            <button type="submit" style={{ background: '#388e3c', color: '#fff', border: 'none', borderBottomRightRadius: 16, padding: '0 18px', fontSize: 18, cursor: 'pointer' }}>Send</button>
          </form>
        </div>
      )}
      ...
      <main style={{
        ...
        <section style={cardStyle}>
          ...
          >
            <span role="img" aria-label="Camera">📷</span> Upload/Scan Crop Photo
          </button>
        ...
        <section style={cardStyle}>
          ...
          <div style={{ display: 'flex', gap: 8, alignItems: 'center', marginBottom: 8 }}>
            ...
            />
            <button
              style={{ ...iconButtonStyle, background: listening ? '#ffeb3b' : '#388e3c', color: listening ? '#222' : '#fff', border: listening ? '2px solid #388e3c' : 'none' }}
      ...
      <footer style={{ textAlign: 'center', color: '#388e3c', marginTop: 40, fontSize: 15, opacity: 0.7 }}>
